...
public class TableUtils {
  ...
  public static void main(String[] args) {
	  ...
	  try{
		  ...
		  MakeMergedMetageneTable("C:/Datas/ColonCancer/analysis/often_communities/nmf_sameICA/");
		  System.exit(0);
  ...
  public static double norm( float[] data )
   ...
   public static int[] Sort(float cais[]){
	...
	public static void ConverTableIntoIdLists(VDataTable vt, String folder, float threshold) throws Exception{
		for(int i=0;i<vt.colCount;i++)if(vt.fieldTypes[i]==vt.NUMERICAL){
			FileWriter fwa = new FileWriter(folder+vt.fieldNames[i]+".txt");
			FileWriter fwp = new FileWriter(folder+vt.fieldNames[i]+"+.txt");
			FileWriter fwm = new FileWriter(folder+vt.fieldNames[i]+"-.txt");
			for(int j=0;j<vt.rowCount;j++){
				String s = vt.stringTable[j][i];
				if(!s.toLowerCase().equals("n/a")){
					float val = Float.parseFloat(s);
					if(Math.abs(val)>threshold) fwa.write(vt.stringTable[j][0]+"\n");
					if(val>threshold) fwp.write(vt.stringTable[j][0]+"\n");
					if(val<-threshold) fwm.write(vt.stringTable[j][0]+"\n");
				}
			}
			fwa.close();
			fwp.close();
	...
	public static void filterColumnsInTxtTable(String fn, String columnAnnotationTable, String feature, String value) throws Exception{
		LineNumberReader lr = new LineNumberReader(new FileReader(fn));
		FileWriter fw = new FileWriter(fn.substring(0,fn.length()-4)+"_copy.txt");
		VDataTable vt = VDatReadWrite.LoadFromSimpleDatFile(columnAnnotationTable, true, "\t");
		vt.makePrimaryHash(feature);
		Vector<Integer> cols = vt.tableHashPrimary.get(value);
		HashSet<String> valColumns = new HashSet<String>();
		for(int i=0;i<cols.size();i++)
			valColumns.add(vt.stringTable[cols.get(i)][0]);
				
		
		String columnNameLine = lr.readLine();
		String colNames[] = columnNameLine.split("\t");
		Set<Integer> indices = new HashSet<Integer>();
		indices.add(0);
		for(int i=0;i<colNames.length;i++)
			if(valColumns.contains(colNames[i]))
				indices.add(i);
   ...
   public static void MakeMergedMetageneTable(String folder) throws Exception{
	   MetageneLoader ml = new MetageneLoader();
	   ml.loadMetagenes(folder);

		Vector allGeneNames = new Vector();
		
		Iterator<String> it = ml.allProbeSets.iterator();
		while(it.hasNext()) allGeneNames.add(it.next());

		Collections.sort(allGeneNames);

			FileWriter fw = new FileWriter(folder+"MergedMetagenes.xls");
			fw.write("GENESYMBOL\t");
			for(int i=0;i<ml.listOfMetagenes.size();i++){
				Metagene mg = ml.listOfMetagenes.get(i);
				fw.write(mg.name+"\t");
			}
			fw.write("\n");
			...
			for(int i=0;i<allGeneNames.size();i++){
				...
				System.out.println(""+i+"/"+allGeneNames.size()+": "+gname);
				fw.write(gname+"\t");
				for(int j=0;j<ml.listOfMetagenes.size();j++){
					Metagene mg = ml.listOfMetagenes.get(j);
					if(!mg.probeSets.contains(gname)){
						fw.write("N/A\t");
					}else{
					int k = mg.probeSets.indexOf(gname);
			...
			VDataTable vt = VDatReadWrite.LoadFromSimpleDatFile(folder+"MergedMetagenes.xls", true, "\t");
			VSimpleProcedures.findAllNumericalColumns(vt);
			VDatReadWrite.saveToVDatFile(vt, folder+"MergedMetagenes.dat");
