...
public class GMTReader {
  ...
  public static void saveSignaturesTOGMTFormat(Vector sig, String fout){
	  saveSignaturesTOGMTFormat(sig,fout,false);
  }
  
  public static void saveSignaturesTOGMTFormat(Vector sig, String fout, boolean writeNamesInsteadOfProbesets){
    try{
      FileWriter fw = new FileWriter(fout);
      ...
      for(int i=0;i<sig.size();i++){
        ...
        if(!writeNamesInsteadOfProbesets){
        	for(int j=0;j<gs.probeSets.size();j++){
        		String s = (String)gs.probeSets.elementAt(j);
        		fw.write("\t"+s);
        	}
        }else{
        	for(int j=0;j<gs.geneNames.size();j++){
        		if(gs.getClass().getName().contains("Metagene")){
        			Metagene mg = (Metagene)gs;
        			String s = (String)mg.geneNames.elementAt(j);
        			float weight = mg.weights.get(j);
        			if(mg.weightSpecified.get(j)){
        				s = s+"["+df.format(weight)+"]";
        			}
        			fw.write("\t"+s);
        		}else{
