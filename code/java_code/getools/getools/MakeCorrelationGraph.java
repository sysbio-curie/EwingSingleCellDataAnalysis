...
public class MakeCorrelationGraph {
	...
	public static void main(String[] args) {
		try{
			...
			if(computeCorrelations){
				...
				System.out.println("Time:"+(new Date().getTime()-t.getTime())+" ms spent.");
			}
			if(assembleGraph){
	...
	public static void MakeCorrelationGraph(String folder, boolean splitTails) throws Exception{
		...
		for(int i=0;i<tables.size();i++){
			...
			if(i!=j){
				...
				if(splitTails){
					Date dt = new Date();
					fr.curie.BiNoM.pathways.utils.Utils.separatePositiveAndNegativeValues(vt1);
					fr.curie.BiNoM.pathways.utils.Utils.separatePositiveAndNegativeValues(vt2);
	...
	public static Graph makeTableCorrelationGraph(VDataTable vt1, String prefix1, VDataTable vt2, String prefix2, float correlationThreshold, boolean chooseOnlyMaximalCorrelation, boolean splitTails) throws Exception{
		...
		// Make two data tables once
		float d1[][] = new float[vt1.colCount-1][names.size()];
		float d2[][] = new float[vt2.colCount-1][names.size()];
		for(int i=1;i<vt1.colCount;i++)for(int j=0;j<names.size();j++){
			String val = vt1.stringTable[vt1.tableHashPrimary.get(names.get(j)).get(0)][i];
			if(val.toLowerCase().equals("n/a"))
				d1[i-1][j] = Float.NaN;
			else
				d1[i-1][j] = Float.parseFloat(val);
		}
		for(int i=1;i<vt2.colCount;i++)for(int j=0;j<names.size();j++){
			String val = vt2.stringTable[vt2.tableHashPrimary.get(names.get(j)).get(0)][i];
			if(val.toLowerCase().equals("n/a"))
				d2[i-1][j] = Float.NaN;
			else
				d2[i-1][j] = Float.parseFloat(val);
		}
		...
		for(int i=0;i<vt1.colCount;i++)if(vt1.fieldTypes[i]==vt1.NUMERICAL){
			...
			for(int j=0;j<vt2.colCount;j++)if(vt2.fieldTypes[j]==vt2.NUMERICAL){
				...
				if(abscorr>=correlationThreshold)if(numberOfPoints>10)if(pval<0.01){
					...
					//System.out.println(fni+"_"+prefix1+"\t"+fnj+"_"+prefix2+"\t"+corr+"\t"+Math.abs(corr));
	...
	public static void SplitAllFilesIntoPositiveAndNegativeTails(String folder) throws Exception{
		...
		for(int i=0;i<fs.length;i++)if(fs[i].getName().endsWith("_S.xls"))if(!fs[i].getName().contains("_positive_"))if(!fs[i].getName().contains("_negative_")){
			...
			VDataTable vtp = fr.curie.BiNoM.pathways.utils.Utils.separatePositiveOrNegativeValues(vt1, true);
			VDataTable vtn = fr.curie.BiNoM.pathways.utils.Utils.separatePositiveOrNegativeValues(vt1, false);
	  ...
	  public static void ComputeResidueFile(String folder, String file, String reference) throws Exception{
		  ...
		  TableUtils.findAllNumericalColumns(vt); vt.makePrimaryHash(vt.fieldNames[0]);
		  TableUtils.findAllNumericalColumns(ref); ref.makePrimaryHash(ref.fieldNames[0]);
		  Vector<String> names = new Vector<String>();
